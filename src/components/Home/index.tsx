...
import { getStories } from "../../store/actions/storyActions"
...
class Home extends Component<props> {
    ...
    showArtciles = (story: tr) => {
        if (story.collection) {
            const rowsArray: IStoryData[][] = this.rowGenerator(
                story.collection,
                2
            )
    ...
    rowGenerator = (lists: Array<IStoryData>, cols: number) => {
        ...
        const articlesRows = rows.map((row: [], i: number) =>
    ...
    generateRowBlocks = (rows: Array<IStoryData[]>, cl: string) =>
        rows.map((row: Array<IStoryData>, i: number) => (
            <div className="row" key={i}>
                {row.map((el: any) => (
                    <div key={el._id} className={`${cl} columns article_block`}>
                        {el.name}
